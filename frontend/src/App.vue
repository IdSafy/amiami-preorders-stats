...
const costPerMonth = computed(() => {
  ...
  const startDate = new Date(Math.min(...tree.value.map(order => new Date(order.data.date))));
  const endDate = new Date(Math.max(...tree.value.map(order => new Date(order.data.date))));
...
const getOrdersData = async () => {
  try {
    ...
    console.error(e)
    toast.add({ severity: 'danger', summary: 'Error', detail: `Failed to fetch data: ${e}` })
    return []
  }
}
...
const postUpdateDataRequest = async () => {
  try {
    ...
    console.error(e)
    toast.add({ severity: 'danger', summary: 'Error', detail: `Failed to update data: ${e}` })
    return []
  }
}
